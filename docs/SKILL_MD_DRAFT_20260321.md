...
2. 只要发现有分页按钮或下一页逻辑，就必须考虑 `nextContentUrl`。
3. 禁止使用 `prevContentUrl`。
4. 禁止使用 `:contains()` 作为 Legado 规则写法。
5. 禁止使用 `:first-child`、`:last-child` 这类不兼容写法，应改用索引方式。
6. 不要把可选增强能力当作默认必走流程。
7. 复杂 JavaScript 必须附用途说明。
8. 最终结果必须提醒用户在 Legado APP 中实测。
...
- `references/css-selector-reference.md`
- `references/css选择器规则.md`
- `references/regex-guide.md`
...
- `references/动态加载.md`
- `references/captcha-detection-guide.md`
- `references/webview-limitations.md`
- `references/方法-登录检查JS.md`
- `references/方法-JS扩展类.md`
