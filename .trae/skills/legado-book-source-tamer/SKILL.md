...
| 模式 | 步骤 | 禁止 |
|------|------|------|
| **查询模式** | 查询知识库 → 回答问题 → 提供示例 | 调用edit_book_source |
| **教学模式** | 查询文档 → 展示原文 → 保持原貌 | 调用edit_book_source |
| **完整生成** | 三阶段：分析→审查→创建 | 前两阶段调用edit_book_source |

### 完整生成模式三阶段

1. **第一阶段**：查询知识库 → 检测编码 → 获取HTML → 分析结构
2. **第二阶段**：审查规则 → 处理特殊情况
3. **第三阶段**：调用edit_book_source → 保存JSON
...
- 不获取真实HTML就编写规则
- 不基于真实HTML结构编写规则
- 前两阶段调用edit_book_source
- 多次调用edit_book_source
...
`nextContentUrl` 字段取决于按钮的**实际功能**，而不是文字：

| 场景 | 是否设置 | 说明 |
|------|----------|------|
| 按钮链接到**真正的下一章节** | ✅ 设置 | 如"下一章"、"下章" |
| 按钮只是**翻页**或**返回目录** | ❌ 不设置 | 如"下一页"、"返回" |
...
| 场景 | 按钮文字 | 功能 | nextContentUrl |
|------|---------|------|----------------|
| 真正的下一章 | "下一章"、"下章" | 跳转到下一章 | `text.下一章@href` |
| 同一章分页 | "下一页" | 同一章分页 | `text.下一页@href` |
| 模糊按钮 | "下一"、"下页" | 需要判断 | `text.下一@href` |
| 无分页按钮 | 无 | 单页正文 | 留空 |

**核心原则**：只要有分页按钮，就必须设置 nextContentUrl！
...
**重要规则**：
- 如果详情页和目录页在同一页面，则 `tocUrl` 字段不需要填写
- 只有当详情页和目录页是分开的两个页面时，才需要填写 `tocUrl`
