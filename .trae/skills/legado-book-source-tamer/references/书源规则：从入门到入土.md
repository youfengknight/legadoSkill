...
* 目录章节重写（高级技巧）
  ...
  当网站目录结构不规范时，可以在 chapterList 规则中使用 JavaScript 重写章节信息：

  **场景**：网站只显示部分章节，需要根据规律生成完整章节列表

  ```javascript
  // chapterList 规则示例
  ".chapter-list.1@a\n@js:\nvar list = [];\nvar p = String(result[0].attr(\"href\")).replace(/\\/\\d+\\.html/, \"/\");\nfor (var i = 0; i < result.length; i++) {\n\tvar ii = i + 1;\n\tvar h = p + ii + \".html\";\n\tvar t = String(result[i].text());\n\tt = t.replace(/第([^章]*)章/, \"第\" + ii + \"章\");\n\tlist.push({text: t, href: h});\n}\nlist"
  ```

  **关键技术**：
  1. 获取第一个章节的URL，提取基础路径
  2. 循环生成所有章节URL（`/1.html`, `/2.html`...）
  3. 重写章节标题中的序号
  4. 返回包含 `text`（章节名）和 `href`（章节URL）的对象数组
